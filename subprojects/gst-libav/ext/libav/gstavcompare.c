...
/**
 * SECTION:element-avcompare
 * @title: avcompare
 * @short_description: A libav based video compare element
 *
 * avcompare accepts two input video streams with the same width, height and
 * format.  The two incoming buffers are compared to each other via the
 * chosen compare method (e.g. ssim or psnr).
 *
 ...
 *   ! avcompare method=psnr name=cmp           \
 ...
 *   ! avcompare method=ssim name=cmp           \
...
#define GST_FFMPEG_COMPARE_FORMATS "{ " \
  "ARGB, BGRA, ABGR, RGBA, xRGB, BGRx, xBGR, RGBx, RGB16, " \
  "GRAY8, NV12, NV21, YUY2, UYVY, I420, Y42B, Y444" \
  " }"

/* *INDENT-OFF* */
static GstStaticPadTemplate gst_ffmpegcompare_src_tmpl =
  GST_STATIC_PAD_TEMPLATE ("src",
    GST_PAD_SRC,
    GST_PAD_ALWAYS,
    GST_STATIC_CAPS (GST_VIDEO_CAPS_MAKE (GST_FFMPEG_COMPARE_FORMATS)));
/* *INDENT-ON* */

/* *INDENT-OFF* */
static GstStaticPadTemplate gst_ffmpegcompare_sink1_tmpl =
...
/* *INDENT-OFF* */
static GstStaticPadTemplate gst_ffmpegcompare_sink2_tmpl =
...
typedef enum
{
  GST_FFMPEGCOMPARE_METHOD_SSIM,
  GST_FFMPEGCOMPARE_METHOD_PSNR,
} GstFFMpegCompareMethod;

#define GST_FFMPEGCOMPARE_METHOD_TYPE (gst_ffmpegcompare_method_get_type())
static GType
gst_ffmpegcompare_method_get_type (void)
{
  static gsize g_type = 0;

  static const GEnumValue enum_values[] = {
    {GST_FFMPEGCOMPARE_METHOD_SSIM, "SSIM", "ssim"},
    {GST_FFMPEGCOMPARE_METHOD_PSNR, "PSNR", "psnr"},
    {0, NULL, NULL},
  };

  if (g_once_init_enter (&g_type)) {
    const GType type =
        g_enum_register_static ("GstFFMpegCompareMethod", enum_values);
...
#define DEFAULT_COMPARE_METHOD GST_FFMPEGCOMPARE_METHOD_SSIM

typedef struct _GstFFMpegCompare GstFFMpegCompare;
typedef struct _GstFFMpegCompareClass GstFFMpegCompareClass;

struct _GstFFMpegCompare
{
  GstElement element;
  ...
  GstFFMpegCompareMethod method;

  gint position;
};

struct _GstFFMpegCompareClass
{
  GstElementClass parent_class;
};

GType gst_ffmpegcompare_get_type (void);

#define GST_TYPE_FFMPEGCOMPARE \
  (gst_ffmpegcompare_get_type())
#define GST_FFMPEGCOMPARE(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj),GST_TYPE_FFMPEGCOMPARE,GstFFMpegCompare))
#define GST_FFMPEGCOMPARE_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_CAST((klass),GST_TYPE_FFMPEGCOMPARE,GstFFMpegCompareClass))
#define GST_IS_FFMPEGCOMPARE(obj) \
  (G_TYPE_CHECK_INSTANCE_TYPE((obj),GST_TYPE_FFMPEGCOMPARE))
#define GST_IS_FFMPEGCOMPARE_CLASS(klass) \
  (G_TYPE_CHECK_CLASS_TYPE((klass),GST_TYPE_FFMPEGCOMPARE))

static void gst_ffmpegcompare_finalize (GObject * object);
static GstFlowReturn gst_ffmpegcompare_collected (GstCollectPads * pads,
    GstFFMpegCompare * self);
static void gst_ffmpegcompare_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_ffmpegcompare_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static GstStateChangeReturn gst_ffmpegcompare_change_state (GstElement *
    element, GstStateChange transition);
static gboolean gst_ffmpegcompare_sink_event (GstCollectPads * pads,
    GstCollectData * data, GstEvent * event, gpointer user_data);

#define gst_ffmpegcompare_parent_class parent_class
G_DEFINE_TYPE (GstFFMpegCompare, gst_ffmpegcompare, GST_TYPE_ELEMENT);

static void
gst_ffmpegcompare_class_init (GstFFMpegCompareClass * klass)
{
  ...
  gobject_class->set_property = gst_ffmpegcompare_set_property;
  gobject_class->get_property = gst_ffmpegcompare_get_property;
  gobject_class->finalize = (GObjectFinalizeFunc) gst_ffmpegcompare_finalize;
  ...
  g_object_class_install_property (gobject_class, PROP_COMPARE_METHOD,
      g_param_spec_enum ("method", "Method", "Method to compare video frames",
          GST_FFMPEGCOMPARE_METHOD_TYPE, DEFAULT_COMPARE_METHOD,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  gstelement_class->change_state =
      GST_DEBUG_FUNCPTR (gst_ffmpegcompare_change_state);

  gst_element_class_add_static_pad_template (gstelement_class,
      &gst_ffmpegcompare_sink1_tmpl);
  gst_element_class_add_static_pad_template (gstelement_class,
      &gst_ffmpegcompare_sink2_tmpl);
  gst_element_class_add_static_pad_template (gstelement_class,
      &gst_ffmpegcompare_src_tmpl);
...
static void
gst_ffmpegcompare_reset (GstFFMpegCompare * self)
{
  self->width = -1;
  self->height = -1;
  self->fps_num = 0;
...
static void
gst_ffmpegcompare_init (GstFFMpegCompare * self)
{
  gst_ffmpegcompare_reset (self);

  self->stats_file = g_strdup (DEFAULT_STATS_FILE);
  self->method = DEFAULT_COMPARE_METHOD;

  self->sinkpad1 =
      gst_pad_new_from_static_template (&gst_ffmpegcompare_sink1_tmpl, "sink1");
  GST_PAD_SET_PROXY_CAPS (self->sinkpad1);
  gst_element_add_pad (GST_ELEMENT (self), self->sinkpad1);

  self->sinkpad2 =
      gst_pad_new_from_static_template (&gst_ffmpegcompare_sink2_tmpl, "sink2");
  GST_PAD_SET_PROXY_CAPS (self->sinkpad2);
  gst_element_add_pad (GST_ELEMENT (self), self->sinkpad2);

  self->srcpad =
      gst_pad_new_from_static_template (&gst_ffmpegcompare_src_tmpl, "src");
  gst_element_add_pad (GST_ELEMENT (self), self->srcpad);

  self->collect = gst_collect_pads_new ();
  gst_collect_pads_set_function (self->collect,
      (GstCollectPadsFunction) GST_DEBUG_FUNCPTR (gst_ffmpegcompare_collected),
      self);
  gst_collect_pads_set_event_function (self->collect,
      GST_DEBUG_FUNCPTR (gst_ffmpegcompare_sink_event), self);
...
static void
gst_ffmpegcompare_finalize (GObject * object)
{
  GstFFMpegCompare *self = GST_FFMPEGCOMPARE (object);
...
static void
gst_ffmpegcompare_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstFFMpegCompare *self = GST_FFMPEGCOMPARE (object);
...
static void
gst_ffmpegcompare_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstFFMpegCompare *self = GST_FFMPEGCOMPARE (object);
...
static gboolean
gst_ffmpegcompare_setcaps (GstPad * pad, GstCaps * caps)
{
  GstFFMpegCompare *self = GST_FFMPEGCOMPARE (GST_PAD_PARENT (pad));
  GstVideoInfo vinfo;
...
static gboolean
gst_ffmpegcompare_sink_event (GstCollectPads * pads, GstCollectData * data,
    GstEvent * event, gpointer user_data)
{
  GstFFMpegCompare *self = GST_FFMPEGCOMPARE (user_data);
  GstPad *pad = data->pad;
  gboolean ret = FALSE;
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_CAPS:
    {
      ...
      ret = gst_ffmpegcompare_setcaps (pad, caps);
...
gst_ffmpegcompare_change_state (GstElement * element, GstStateChange transition)
{
  GstFFMpegCompare *self = GST_FFMPEGCOMPARE (element);
...
static gint
init_filter_graph (GstFFMpegCompare * self)
{
  ...
  enum_class = g_type_class_ref (GST_FFMPEGCOMPARE_METHOD_TYPE);
...
static gint
process_filter_graph (GstFFMpegCompare * self, AVFrame * in1, AVFrame * in2)
{
  AVFrame *out;
  gint res;
...
static void
_fill_avpicture (GstFFMpegCompare * self, AVFrame * picture,
...
gst_ffmpegcompare_collected (GstCollectPads * pads, GstFFMpegCompare * self)
{
...
gboolean
gst_ffmpegcompare_register (GstPlugin * plugin)
{
  return gst_element_register (plugin, "avcompare", GST_RANK_NONE,
      GST_TYPE_FFMPEGCOMPARE);
}
